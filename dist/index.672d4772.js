...
addTask.addEventListener("submit", function(e) {
    ...
    if (taskText === "") return;
    createTask(taskText, String(new Date().getTime()).slice(-5).padStart(5, 0), false, false);
    localTasks.push({
        taskText,
...
const createTask = function(textTask, taskIndex = String(new Date().getTime()).slice(-5).padStart(5, 0), done = false, isLocal = false) {
    const markup = `          
...
tasks.addEventListener("click", function(e) {
    ...
    if (!e.target.closest(".remove-task")) {
        ...
        const findTaskIndex = localTasks.findIndex((t)=>t.index === taskDone.dataset.index.padStart(5, 0) || t.index === String(+taskDone.dataset.index + 1).padStart(5, 0));
        if (findTaskIndex >= 0) localTasks[findTaskIndex].done = !localTasks[findTaskIndex].done;
    ...
    if (e.target.closest(".remove-task")) {
        ...
        localTasks.splice(localTasks.findIndex((ind)=>+ind.index === +selectTask.dataset.index), 1);
        selectTask.classList.add("hidden");
        updateStorage();
        setTimeout(()=>{
            selectTask.remove();
            counterUpdate();
            if (!tasks.querySelector(".task")) createTask("Add your daily tasks", undefined, false, false);
        }, 300);
    }
});
...
function counterUpdate() {
    ...
    const counter = doneTasks.reduce((curr, don)=>curr + don.classList.contains("done-task"), 0);
...
function removeFirstTask() {
    ...
    selectTasks.forEach((selectTask)=>{
        if (selectTask.textContent === "Add your daily tasks") selectTask.closest(".task").remove();
        counterUpdate();
    });
}
counterUpdate();
